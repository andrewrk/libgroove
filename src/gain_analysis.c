...
GainAnalysis * gain_create_analysis(long samplefreq) {
    ...
    a->rstep        = a->rstepbuf  + MAX_ORDER;
    a->lout         = a->loutbuf   + MAX_ORDER;
        ...
        const double* right_samples, size_t num_samples, int num_channels )
{
    ...
    if ( num_samples < MAX_ORDER ) {
        memcpy ( a->linprebuf + MAX_ORDER, left_samples , num_samples * sizeof(double) );
        memcpy ( a->rinprebuf + MAX_ORDER, right_samples, num_samples * sizeof(double) );
    }
    else {
        memcpy ( a->linprebuf + MAX_ORDER, left_samples,  MAX_ORDER   * sizeof(double) );
        memcpy ( a->rinprebuf + MAX_ORDER, right_samples, MAX_ORDER   * sizeof(double) );
    }

    while ( batchsamples > 0 ) {
        cursamples = batchsamples > a->sampleWindow - a->totsamp  ?  a->sampleWindow - a->totsamp  :  batchsamples;
        if ( cursamplepos < MAX_ORDER ) {
            curleft  = a->linpre+cursamplepos;
            curright = a->rinpre+cursamplepos;
        ...
        filterYule ( curleft , a->lstep + a->totsamp, cursamples, ABYule[a->freqindex]);
        filterYule ( curright, a->rstep + a->totsamp, cursamples, ABYule[a->freqindex]);

        filterButter ( a->lstep + a->totsamp, a->lout + a->totsamp, cursamples, ABButter[a->freqindex]);
        filterButter ( a->rstep + a->totsamp, a->rout + a->totsamp, cursamples, ABButter[a->freqindex]);

        curleft = a->lout + a->totsamp;                   // Get the squared values
        curright = a->rout + a->totsamp;

        i = cursamples % 16;
        while (i--)
        {   a->lsum += fsqr(*curleft++);
            a->rsum += fsqr(*curright++);
        }
        i = cursamples / 16;
        while (i--)
        {   a->lsum += fsqr(curleft[0])
                  + fsqr(curleft[1])
                  + fsqr(curleft[2])
                  + fsqr(curleft[3])
                  + fsqr(curleft[4])
        ...
        a->totsamp      += cursamples;
        if ( a->totsamp == a->sampleWindow ) {  // Get the Root Mean Square (RMS) for this set of samples
            ...
            memmove ( a->routbuf , a->routbuf  + a->totsamp, MAX_ORDER * sizeof(double) );
            memmove ( a->lstepbuf, a->lstepbuf + a->totsamp, MAX_ORDER * sizeof(double) );
            memmove ( a->rstepbuf, a->rstepbuf + a->totsamp, MAX_ORDER * sizeof(double) );
            a->totsamp = 0;
        }
        if (a->totsamp > a->sampleWindow )   // somehow I really screwed up: Error in programming! Contact author about totsamp > sampleWindow
            return GAIN_ANALYSIS_ERROR;
    }
    if ( num_samples < MAX_ORDER ) {
        memmove ( a->linprebuf,                           a->linprebuf + num_samples, (MAX_ORDER-num_samples) * sizeof(double) );
        memmove ( a->rinprebuf,                           a->rinprebuf + num_samples, (MAX_ORDER-num_samples) * sizeof(double) );
        memcpy  ( a->linprebuf + MAX_ORDER - num_samples, left_samples,          num_samples             * sizeof(double) );
        memcpy  ( a->rinprebuf + MAX_ORDER - num_samples, right_samples,         num_samples             * sizeof(double) );
    }
    else {
...
double gain_get_chapter(GainAnalysis *anal) {
    ...
    for ( i = 0; i < MAX_ORDER; i++ )
        a->linprebuf[i] = a->lstepbuf[i] = a->loutbuf[i] = a->rinprebuf[i] = a->rstepbuf[i] = a->routbuf[i] = 0.f;
